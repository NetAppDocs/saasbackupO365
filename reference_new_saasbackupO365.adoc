...
* Customers who only have OneDrive for Business licenses can release licenses and purge users without restrictions.
+
link:task_releasing_a_user_license.html[Releasing a user license]
link:task_purging.html[Purging]

* You can now filter by job completion status in addition to job type, service, start time, and end time when you search in the job history log.
+
