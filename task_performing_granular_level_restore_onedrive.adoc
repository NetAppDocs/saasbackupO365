...
+
.Note
If you want to restore individual folders or files within a group, click on the group of files and then select the individual folders or files inside the group. To restore an entire folder, select the folder. To restore individual files within the folder, select the folder containing the files, and then select the individual files within the folder.
. Click *Restore*.
+
...
If you restore to another MySite, you must enter the destination MySite in the search field.  You can type in a portion of the destination MySite in the search field to initiate an automatic search for matching destination MySites.
  ...
  * Export
+
Note: This option is only available at the folder level.  It is not available for individual files.  If you select to export a folder, an option appears to include subfolders, if desired.
+
